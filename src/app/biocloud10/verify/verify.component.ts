...
export class VerifyComponent implements OnInit {
  ...
  public res: any;
  public bioCloudVerifyForm: FormGroup;
  isV12 = true;
  isV12Track = true;
  ...
  verifyReqModel: CommonBioReqV10Model;
  verifyResModel: BioResV10Model;
  ...
  ngOnInit(): void {
    this.titleService.setTitle(Common.BIOCLOUD_V10_VERIFY_TITLE);
  ...
  createForm() {
    this.bioCloudVerifyForm = this.formBuilder.group({
  ...
  initializeFormData() {
    try {
      ...
      if (!this.isV12) {
        ...
        this.bioCloudVerifyForm.get('deviceName').setValue(cookieDeviceName);
      } else {
        this.notifyService.showError(
          MessageConstants.DEVICE_NOT_FOUND_ERROR_MSG,
          MessageConstants.BIOCLOUD_V10_VERIFY_ERROR_TITLE
        );
      }
    } catch (error) {
      this.notifyService.showError(
  ...
  prepareCaptureRequest() {
    let formData = this.bioCloudVerifyForm.value;
    const engine = this.getEngineNameByDevice(
      this.bioCloudVerifyForm.controls['deviceName'].value
    );
    var captureReqModel = {};
    try {
      if (engine === EnumEnginesMapper.FVHT01) {
        captureReqModel = {
          ...
          DeviceName: this.bioCloudVerifyForm.controls['deviceName'].value,
          CaptureTimeOut: 180.0,
          FaceImage: EnumFeatureMode.Disable,
          CaptureType: this.bioCloudVerifyForm.controls['captureType'].value,
  ...
  prepareVerifyRequestModel() {
    ...
    try {
      const verifyReqModel = {
        ...
        RegistrationId:
          this.bioCloudVerifyForm.controls['registrationNo'].value,
        BiometricXml: this.biometricXml,
        Format: Common.ISO_FORMAT,
        EngineName: cookieEngineName,
      };
      this.verifyReqModel = new CommonBioReqV10Model(verifyReqModel);
    } catch (error) {
      this.notifyService.showError(
        error.error.message,
        MessageConstants.GENERAL_ERROR_TITLE
  ...
  onSubmit() {
    ...
    if (this.bioCloudVerifyForm.invalid) {
      return;
    }
